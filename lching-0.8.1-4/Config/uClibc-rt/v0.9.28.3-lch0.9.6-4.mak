# uClibc-rt 0.9.28.3		[ since 0.9.20, c. 2002-10-14 ]
# last mod WmT, 2011-03-15	[ (c) and GPLv2 1999-2011 ]
...
URLS+=http://uclibc.org/downloads/old-releases/uClibc-${UCLIBC_RT_VER}.tar.bz2
...
ifeq (${UCLIBC_RT_VER},0.9.28.3)
#UCLIBC_DEV_SRC+=${SOURCEROOT}/u/uClibc-0.9.28.3-patches-1.3.tar.bz2
UCLIBC_DEV_SRC+=${SOURCEROOT}/u/uClibc-0.9.28.3-patches-1.8.tar.bz2
#UCLIBC_DEV_SRC+=${SOURCEROOT}/u/uClibc-0.9.28.3-patches-1.9.tar.bz2
#URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/uClibc-0.9.28.3-patches-1.3.tar.bz2
URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/uClibc-0.9.28.3-patches-1.8.tar.bz2
#URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/uClibc-0.9.28.3-patches-1.9.tar.bz2
endif
...
UCLIBC_RT_INSTTEMP:=${EXTTEMP}/uClibc-rt-${UCLIBC_RT_VER}-insttemp
UCLIBC_RT_EGPNAME:=uClibc-rt-${UCLIBC_RT_VER}
...
${UCLIBC_RT_INSTTEMP}/lib/libc.so.0:
	${MAKE} ${EXTTEMP}/uClibc-rt-${UCLIBC_RT_VER}-xdc/lib/libc.a
	mkdir -p ${UCLIBC_RT_INSTTEMP}/sbin || exit 1
	( cd ${EXTTEMP}/uClibc-rt-${UCLIBC_RT_VER}-xdc || exit 1 ;\
		case ${UCLIBC_RT_VER} in \
		0.9.20) \
			${MAKE} PREFIX=${UCLIBC_RT_INSTTEMP} install_target || exit 1 ;\
			cp ldso/util/ldconfig ${UCLIBC_RT_INSTTEMP}/sbin || exit 1 \
		;; \
		0.9.26) \
			${MAKE} PREFIX=${UCLIBC_RT_INSTTEMP} install_runtime || exit 1 \
		;; \
		0.9.2[89]*) \
			make PREFIX=${UCLIBC_RT_INSTTEMP}/ RUNTIME_PREFIX='/' install_runtime || exit 1 ;\
			cp utils/ldconfig ${UCLIBC_RT_INSTTEMP}/sbin || exit 1 \
		;; \
		*) \
...
${TOPLEV}/${UCLIBC_RT_EGPNAME}.egp: ${UCLIBC_RT_INSTTEMP}/lib/libc.so.0
	${PCREATE_SCRIPT} create ${TOPLEV}/${UCLIBC_RT_EGPNAME}.egp ${UCLIBC_RT_INSTTEMP}

# uClibc-rt: 'egp' maintains the dates from the archive
${XDC_ROOT}/lib/libc.so.0: ${TOPLEV}/${UCLIBC_RT_EGPNAME}.egp
	mkdir -p ${XDC_ROOT}
	STRIP=${TARGET_SPEC}-strip ${PCREATE_SCRIPT} install ${XDC_ROOT} ${TOPLEV}/${UCLIBC_RT_EGPNAME}.egp
	touch $@

REALCLEAN_TARGETS+= ${TOPLEV}/${UCLIBC_RT_EGPNAME}.egp
...
.PHONY: xdc-uClibc-rt
ifeq (${MAKE_CHROOT},y)
xdc-uClibc-rt: ${XDC_ROOT}/lib/libc.so.0
else
xdc-uClibc-rt: ${TOPLEV}/${UCLIBC_RT_EGPNAME}.egp
endif
