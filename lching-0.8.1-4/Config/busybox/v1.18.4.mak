# busybox 1.18.4		[ since 0.60.5, c.2006-06-17 ]
...
ifeq (${BUSYBOX_VER},1.11.1)
BUSYBOX_SRC+=${SOURCEROOT}/b/busybox-1.11.1-basename.patch
endif

BUSYBOX_PATH:=${BUSYBOX_PKG}-${BUSYBOX_VER}
...
ifeq (${BUSYBOX_VER},1.11.1)
URLS+=http://busybox.net/downloads/fixes-1.11.1/busybox-1.11.1-basename.patch
endif
	...
	( cd ${EXTTEMP}/${BUSYBOX_PATH}-xdc || exit 1 ;\
		(	case ${BUSYBOX_VER} in \
			...
			1.4.[012]) \
				...
				echo 'CONFIG_FEATURE_COMMAND_TAB_COMPLETION=y' \
			;; \
			1.5.[01]|1.6.0|1.7.[02]|1.8.[012]|1.9.[012]|1.10.[01234]|1.11.[123]|1.12.1|1.13.[1234]|1.14.[0124]|1.15.[23]|1.16.2) \
				echo 'CONFIG_PREFIX="'${BUSYBOX_INSTTEMP}'"' ;\
				echo 'CONFIG_FEATURE_EDITING=y' ;\
				echo 'CONFIG_FEATURE_EDITING_FANCY_KEYS=y' ;\
				echo 'CONFIG_FEATURE_TAB_COMPLETION=y' \
			;; \
			1.17.[01234]|1.18.[0234]) \
				echo 'CONFIG_PREFIX="'${BUSYBOX_INSTTEMP}'"' ;\
				echo 'CONFIG_FEATURE_EDITING=y' ;\
				echo 'CONFIG_FEATURE_EDITING_FANCY_KEYS=y' ;\
				echo 'CONFIG_FEATURE_TAB_COMPLETION=y' ;\
			...
			*) \
			...
			;; \
			1.7.0|1.17.[34]|1.18.[0234]) ;; \
			1.17.[12]) \
				echo '# CONFIG_IP is not set' \
				echo '# CONFIG_IPLINK is not set' \
			;;\
			*) \
		...
		1.5.[01]|1.6.0) \
			...
			cat Makefile.OLD \
				...
				> Makefile || exit 1 ;\
		;; \
		1.7.[02]|1.8.[012]|1.9.[012]|1.10.[01234]|1.11.[123]|1.12.1|1.13.[1234]|1.14.[0124]|1.15.[23]|1.16.2) \
			[ -r Makefile.OLD ] || mv Makefile Makefile.OLD ;\
			cat Makefile.OLD \
				| sed	' /^ARCH/	s%=.*%= '${TARGET_CPU}'%' \
				| sed	' /^CROSS_COMPILE/	s%=.*%= '${XTC_ROOT}'/usr/bin/'${TARGET_SPEC}'-%' \
...
${EXTTEMP}/${BUSYBOX_PATH}-xdc/busybox: ${EXTTEMP}/${BUSYBOX_PATH}-xdc/Makefile
	( cd ${EXTTEMP}/${BUSYBOX_PATH}-xdc || exit 1 ;\
		...
		1.1.0|1.2.2.1) \
			make VERBOSE=y \
		;; \
		1.4.[012]|1.5.[01]|1.6.0|1.7.[02]|1.8.[012]|1.9.[012]|1.10.[01234]|1.11.[123]|1.12.1|1.13.[1234]|1.14.[0124]|1.15.[23]|1.16.2|1.17.[01234]|1.18.[0234]) \
...
${BUSYBOX_INSTTEMP}/bin/busybox: ${EXTTEMP}/${BUSYBOX_PATH}-xdc/busybox
...
REALCLEAN_TARGETS+= ${BUSYBOX_EGPNAME}.egp
