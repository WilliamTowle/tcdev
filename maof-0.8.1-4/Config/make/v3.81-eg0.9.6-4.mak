# make 3.81			[ since v3.79.1, c.2006-06-15 ]
...
MAKE_INSTTEMP:=${EXTTEMP}/${MAKE_PATH}-insttemp
MAKE_EGPNAME:=${MAKE_PKG}-${MAKE_VER}
...
${MAKE_INSTTEMP}/usr/bin/make:
	${MAKE} ${EXTTEMP}/${MAKE_PATH}-xdc/make
	( cd ${EXTTEMP}/${MAKE_PATH}-xdc || exit 1 ;\
		make DESTDIR=${MAKE_INSTTEMP} install || exit 1 ;\
		cd ${MAKE_INSTTEMP}/usr/bin && ln -sf make gmake \
	) || exit 1

${TOPLEV}/${MAKE_EGPNAME}.egp: ${MAKE_INSTTEMP}/usr/bin/make
	${PCREATE_SCRIPT} create ${TOPLEV}/${MAKE_EGPNAME}.egp ${MAKE_INSTTEMP}

${XDC_ROOT}/usr/bin/make: ${TOPLEV}/${MAKE_EGPNAME}.egp
	mkdir -p ${XDC_ROOT}
	STRIP=${TARGET_SPEC}-strip ${PCREATE_SCRIPT} install ${XDC_ROOT} ${TOPLEV}/${MAKE_EGPNAME}.egp

REALCLEAN_TARGETS+= ${TOPLEV}/${MAKE_EGPNAME}.egp
...
.PHONY: xdc-make
ifeq (${MAKE_CHROOT},y)
xdc-make: ${XDC_ROOT}/usr/bin/make
else
xdc-make: ${TOPLEV}/${MAKE_EGPNAME}.egp
endif
