...
# last mod WmT, 2007-08-01	[ (c) and GPLv2 1999-2007 ]
...
${XDC_INSTTEMP}/sbin/fdisk:
	...
	( cd ${EXTTEMP}/${UTILLX_PATH}-xdc || exit 1 ;\
		...
		make DESTDIR=${EXTTEMP}/${UTILLX_PATH}-xdc/insttemp \
			USE_TTY_GROUP=no \
			install || exit 1 ;\
		mkdir -p ${XDC_INSTTEMP}/etc ;\
		cp insttemp/etc/fdprm ${XDC_INSTTEMP}/etc/ ;\
		mkdir -p ${XDC_INSTTEMP}/usr/bin ;\
		cp insttemp/usr/bin/setfdprm ${XDC_INSTTEMP}/usr/bin/ ;\
		cp insttemp/usr/bin/fdformat ${XDC_INSTTEMP}/usr/bin/ ;\
		mkdir -p ${XDC_INSTTEMP}/sbin ;\
		cp insttemp/sbin/fdisk ${XDC_INSTTEMP}/sbin/ \
	) || exit 1

${TOPLEV}/${UTILLX_PKG}-${UTILLX_VER}.egp:
	${MAKE} ${XDC_INSTTEMP}/sbin/fdisk
#	tar cvzf ${UTILLX_PKG}-${UTILLX_VER}.tgz -C ${INSTTEMP} ./
	${PCREATE_SCRIPT} create ${UTILLX_PKG}-${UTILLX_VER}.egp ${INSTTEMP}
	rm -rf ${INSTTEMP}
...
ifeq (${MAKE_CHROOT},y)
xdc-util-linux: ${XDC_INSTTEMP}/sbin/fdisk
else
xdc-util-linux: ${TOPLEV}/${UTILLX_PKG}-${UTILLX_VER}.egp
endif
