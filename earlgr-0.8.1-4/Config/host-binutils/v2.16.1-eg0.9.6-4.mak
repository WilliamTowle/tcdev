# binutils 2.16.1		[ since v2.13, 2002-10-14 ]
...
#HOST_BINUTILS_VER:=2.19

HOST_BINUTILS_PATH:=binutils-${HOST_BINUTILS_VER}
HOST_BINUTILS_INSTTEMP:=${EXTTEMP}/host-binutils-${HOST_BINUTILS_VER}
...
URLS+=http://www.mirrorservice.org/sites/ftp.gnu.org/gnu/binutils/binutils-${HOST_BINUTILS_VER}.tar.bz2

ifeq (${HOST_BINUTILS_VER},2.16.1)
# NB! Gentoo "v1.11" patches are not all relative to the same directory
HOST_BINUTILS_PATCHES+=${SOURCEROOT}/b/binutils-2.16.1-patches-1.11.tar.bz2
URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/binutils-2.16.1-patches-1.11.tar.bz2
HOST_BINUTILS_PATCHES+=${SOURCEROOT}/b/binutils-2.16.1-uclibc-patches-1.1.tar.bz2
...
HOST_BINUTILS_PATCHES+=${SOURCEROOT}/b/binutils-2.17-patches-1.6.tar.bz2
URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/binutils-2.17-patches-1.6.tar.bz2
HOST_BINUTILS_PATCHES+=${SOURCEROOT}/b/binutils-2.17-uclibc-patches-1.0.tar.bz2
...
HOST_BINUTILS_PATCHES+=${SOURCEROOT}/b/binutils-2.18-patches-1.10.tar.bz2
URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/binutils-2.18-patches-1.10.tar.bz2
endif

ifeq (${HOST_BINUTILS_VER},2.19)
# NB! Gentoo "v1.1" patches are not all relative to the same directory
HOST_BINUTILS_PATCHES+=${SOURCEROOT}/b/binutils-2.19-patches-1.1.tar.bz2
URLS+=http://www.mirrorservice.org/sites/www.ibiblio.org/gentoo/distfiles/binutils-2.19-patches-1.1.tar.bz2
endif
...
ifneq (${HOST_BINUTILS_PATCHES},)
	( cd ${EXTTEMP} || exit 1 ;\
		make -C ../ extract LIST="$(strip ${HOST_BINUTILS_PATCHES})" || exit 1 ;\
...
ifeq (${ETCDIR},${XTC_ROOT}/etc/geex)
	( cd ${EXTTEMP} || exit 1 ;\
		...
		for PF in patch/*patch ; do \
			echo "*** PATCHING -- $${PF} ***" ;\
			grep '+++' $${PF} ;\
			sed '/+++ / { s%avr-%% ; s%binutils[^/]*/%% ; s%src/%% }' $${PF} | patch --batch -d ${HOST_BINUTILS_PATH} -Np0 ;\
			rm -f $${PF} ;\
		done \
	) || exit 1
endif
