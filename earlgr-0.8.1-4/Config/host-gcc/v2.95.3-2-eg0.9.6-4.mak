# gcc 2.95.3			[ since v2.7.2.3, c. 2003-10-31 ]
...
#HOST_GCC_PATCHES+=${SOURCEROOT}/g/gcc-${HOST_GCC_VER}-2.patch
#URLS+=http://www.linuxfromscratch.org/patches/downloads/gcc/gcc-2.95.3-2.patch
HOST_GCC_PATCHES+=${SOURCEROOT}/g/gcc-2.95.3-patches-1.3.tar.bz2
...
ifneq (${HOST_GCC_PATCHES},)
	( cd ${EXTTEMP} || exit 1 ;\
		make -C ../ extract LIST="$(strip ${HOST_GCC_PATCHES})" || exit 1 ;\
		for PF in patch/*patch ; do \
			echo "*** PATCHING -- $${PF} ***" ;\
			grep '+++' $${PF} ;\
			patch --batch -d gcc-${HOST_GCC_VER} -Np0 < $${PF} ;\
			rm -f $${PF} ;\
		done \
	) || exit 1
endif
