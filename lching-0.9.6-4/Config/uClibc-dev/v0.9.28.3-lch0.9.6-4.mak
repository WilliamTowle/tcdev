# uClibc-dev 0.9.28.3		[ since 0.9.20, c. 2002-10-14 ]
...
${XTC_ROOT}/usr/bin/${TARGET_SPEC}-ldd:
	...
	( cd ${EXTTEMP}/uClibc-dev-${UCLIBC_DEV_VER}-xtc || exit 1 ;\
		...
		;; \
		0.9.26) \
			cp utils/ldd ${XTC_ROOT}/usr/bin/${TARGET_SPEC}-ldd || exit 1 \
		;; \
		0.9.28*) \
			cp utils/ldd.host ${XTC_ROOT}/usr/bin/${TARGET_SPEC}-ldd || exit 1 \
		;; \
		esac \
	) || exit 1
...
${XDC_ROOT}/usr/lib/libc.a: ${TOPLEV}/${UCLIBC_DEV_EGPNAME}.egp
	mkdir -p ${XDC_ROOT}
	STRIP=${TARGET_SPEC}-strip ${PCREATE_SCRIPT} install ${XDC_ROOT} ${TOPLEV}/${UCLIBC_DEV_EGPNAME}.egp
	touch $@
