...
ifeq (${CROSS_BINUTILS_VER},2.19)
# NB! Gentoo "v1.1" patches are not all relative to the same directory
CROSS_BINUTILS_SRC+=${SOURCEROOT}/b/binutils-2.19-patches-1.1.tar.bz2
	...
	( cd ${EXTTEMP} || exit 1 ;\
		...
		if [ -d patch ] ; then \
			for PF in patch/*patch ; do \
				echo "*** PATCHING -- $${PF} ***" ;\
				grep '+++' $${PF} ;\
				sed '/+++ binutils/ { s%binutils-[^/]*/%% ; s%binutils/ld%ld% }' $${PF} | patch --batch -d ${CROSS_BINUTILS_PATH} -Np0 ;\
				rm -f $${PF} ;\
			done ;\
		fi \
	) || exit 1
...
ifeq (${CROSS_BINUTILS_VER},2.18)
	( cd ${EXTTEMP}/${CROSS_BINUTILS_PATH}-xtc || exit 1 ;\
		...
		cat Makefile.OLD \
			| sed '/^MAKEINFO/ s%/.*%true%' \
			> Makefile ;\
		mv ld/genscripts.sh ld/genscripts.sh.OLD || exit 1 ;\
		cat ld/genscripts.sh.OLD \
			| sed "/local lineno/ { s/^/eval '/ ; s/$$/';/ }" \
			> ld/genscripts.sh \
	) || exit 1
endif
