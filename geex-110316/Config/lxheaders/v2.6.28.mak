# linux headers 2.6.28		[ since 2.0.37pre10 ]
# last mod WmT, 2009-06-03	[ (c) and GPLv2 1999-2009 ]
...
LXHEADERS_INSTTEMP:=${EXTTEMP}/lxheaders-${LXHEADERS_VER}-insttemp
LXHEADERS_EGPNAME:=lxheaders-${LXHEADERS_VER}

URLS+=http://www.mirrorservice.org/sites/ftp.kernel.org/pub/linux/kernel/v2.6/linux-${LXHEADERS_VER}.tar.bz2
...
# not 2.6.x? LXHEADERS_ARCH_OPTS+=CONFIG_SHELL=${CONFIG_SHELL}
...
# v2.6.27 (and earlier?) have new 'arch' substructure
...
${EXTTEMP}/lxheaders-${LXHEADERS_VER}/include/asm: ${EXTTEMP}/lxheaders-${LXHEADERS_VER}/.config
	( cd ${EXTTEMP}/lxheaders-${LXHEADERS_VER} || exit 1 ;\
		case "${LXHEADERS_VER}" in \
		2.0.*|2.2.*|2.4.*) \
...
${ETCDIR}/linux-${LXHEADERS_VER}-config:
	${MAKE} ${EXTTEMP}/lxheaders-${LXHEADERS_VER}/include/asm
	mkdir -p ${ETCDIR}
	( cd ${EXTTEMP}/lxheaders-${LXHEADERS_VER} || exit 1 ;\
...
${XTC_ROOT}/usr/${TARGET_SPEC}/usr/src/linux-${LXHEADERS_VER}:
	...
	( cd ${EXTTEMP}/lxheaders-${LXHEADERS_VER} || exit 1 ;\
		if [ -d arch/x86 ] ; then \
			make headers_install INSTALL_HDR_PATH=${XTC_ROOT}/usr/${TARGET_SPEC}/usr/ ;\
		else \
...
${LXHEADERS_INSTTEMP}/usr/include/linux:
	...
	( cd ${EXTTEMP}/lxheaders-${LXHEADERS_VER} || exit 1 ;\
		if [ -d arch/x86 ] ; then \
			make headers_install INSTALL_HDR_PATH=${LXHEADERS_INSTTEMP}/usr/ ;\
		else \
